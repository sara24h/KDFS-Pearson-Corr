...
from data.video_data import create_uadfv_dataloaders  # این خط رو حتماً اضافه کن
...
class Test:
    ...
    def build_models(self):
        print("==> بارگذاری مدل معلم (فقط برای مقایسه FLOPs)...")
        ...
        # حذف پیشوند module. اگر از DDP استفاده کرده باشی
        if list(state_dict.keys())[0].startswith('module.'):
            from collections import OrderedDict
            new_state_dict = OrderedDict()
            for k, v in state_dict.items():
                new_state_dict[k[7:]] = v  # حذف "module."
            state_dict = new_state_dict

        student.load_state_dict(state_dict, strict=True)
        student.to(self.device)
        student.eval()
        student.ticket = True  # مهم: پرونینگ نهایی فعال شود
        return teacher, student

    def test(self):
        teacher, student = self.build_models()

        # یک فریم نمونه برای محاسبه FLOPs
        dummy = torch.randn(1, 3, 256, 256).to(self.device)

        # FLOPs یک فریم از مدل دانشجو
        flops_frame, params = profile(student, inputs=(dummy,), verbose=False)
        flops_video = flops_frame * self.num_frames / 1e9  # تبدیل به GFLOPs
        ...
        correct = 0
        total = 0
        with torch.no_grad():  # این خط رو اصلاح کردم
            with tqdm(test_loader, desc="Testing Accuracy", ncols=100) as pbar:
        ...
        print(f"کاهش FLOPs نسبت به معلم: {reduction:.2f}% → یعنی فقط {flops_video:.2f} GFLOPs!")
...
class Args:
    def __init__(self):
        ...
        self.sparsed_student_ckpt_path = "/kaggle/working/results/run_resnet50_imagenet_prune1/student_model/resnet50_sparse_best.pt"  # مسیر خودت
        self.num_frames = 16
