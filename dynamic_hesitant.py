...
def create_dataloaders_ddp(base_dir: str, batch_size: int, rank: int, world_size: int, num_workers: int = 2):
    ...
    loaders = {}
    for split, ds in datasets_dict.items():
        # برای همه splits از DistributedSampler استفاده می‌کنیم
        shuffle = (split == 'train')
...
def evaluate_single_model_ddp(model, loader, device, name, rank, world_size):
    ...
    # همه rank‌ها بدون tqdm کار می‌کنند
...
def train_hesitant_fuzzy_ddp(ensemble_model, train_loader, val_loader, num_epochs, lr, device, save_dir, rank, world_size):
    ...
    for epoch in range(num_epochs):
        ...
        # جمع‌آوری متریک‌ها از همه GPU‌ها
...
def main():
    ...
    # بازنویسی seed اگر از CLI داده شده
    if args.seed != SEED:
        set_seed(args.seed)
    ...
    # ارزیابی مدل‌های تک قبل از آموزش
    dist.barrier()
    ...
    # همه rank‌ها این کار را انجام می‌دهند
    individual_accs = []
    for i, model in enumerate(base_models):
        if is_main:
            print(f"\n[Rank {rank}] Evaluating Model {i+1}/{len(base_models)}...")
    ...
    # فقط rank 0 نتایج را نمایش می‌دهد
    if is_main:
...
def main():
    ...
    if os.path.exists(ckpt_path):
        ...
        if is_main:
            print("\n✓ Best hesitant fuzzy network loaded.\n")
  
    dist.barrier()
  
    # ارزیابی نهایی ensemble
    if is_main:
        print("\n" + "="*70)
        print("EVALUATING FUZZY HESITANT ENSEMBLE ON TEST SET")
        print("="*70)
