...
def create_dataloaders_ddp(base_dir: str, batch_size: int, rank: int, world_size: int,
    ...
    # ابعاد تقریبی دیتاست‌ها (بر اساس توضیح شما)
    if dataset_type == 'wild':
        input_size = 224  # کوچک‌تر از 256، upscale ساده
    elif dataset_type == 'real_fake':
        input_size = 600  # بزرگ‌تر، نیاز به کراپ
    elif dataset_type == 'hard_fake_real':
        input_size = 300  # متوسط، نیاز به کراپ ملایم
    else:
        input_size = 256  # دیفالت

    # ترانسفورم train: اگر تصاویر بزرگ‌تر باشن، از RandomResizedCrop استفاده کنید
    if input_size > 256:
        train_transform = transforms.Compose([
            transforms.Resize(int(256 * 1.1)),  # اول کمی بزرگ‌تر ریسایز (مثل 282)
            transforms.RandomResizedCrop(256, scale=(0.8, 1.0)),  # کراپ رندوم برای حفظ جزئیات
        ...
        train_transform = transforms.Compose([
            transforms.Resize((256, 256)),  # ریسایز ساده برای تصاویر کوچک‌تر
            transforms.RandomHorizontalFlip(p=0.5),
            transforms.RandomRotation(10),
            transforms.ColorJitter(0.2, 0.2),
            transforms.ToTensor(),
        ])
    # ترانسفورم val/test: ساده‌تر، با CenterCrop برای حفظ مرکز تصویر
    if input_size > 256:
        val_test_transform = transforms.Compose([
            transforms.Resize(int(256 * 1.1)),
