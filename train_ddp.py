...
class TrainDDP: # اصلاح کوچک در نام کلاس
    ...
    def build_model(self):
        ...
        if self.arch == 'resnet50':
            ...
            # Use ::standard MobileNetV2 architecture for the teacher
    ...
    def define_loss(self):
        ...
        self.sparsity_loss = loss.TargetSparsityLoss(target_retention_ratio=self.target_retention_ratio,coef_sparse=self.coef_sparse).cuda()
    ...
    def train(self):
        ...
        if self.rank == 0:
            ...
            # تعریف متر برای زیان تنکی جدید
            meter_sparsityloss = meter.AverageMeter("SparsityLoss", ":.4e")
        ...
        for epoch in range(self.start_epoch + 1, self.num_epochs + 1):
            ...
            with tqdm(total=len(self.train_loader), ncols=100, disable=self.rank != 0) as _tqdm:
                ...
                for images, targets in self.train_loader:
                    ...
                    with autocast():
                        ...
                        # محاسبه زیان تنکی جدید
                        sparsity_loss_val = self.sparsity_loss(self.student.module)
                        ...
                        total_loss = (
                            ...
                            # اضافه کردن زیان جدید به زیان کل
                            + sparsity_loss_val
                        )
                    ...
                    # کاهش زیان جدید بین GPUها
                    reduced_sparsity_loss = self.reduce_tensor(sparsity_loss_val)
                    ...
                    if self.rank == 0:
                        ...
                        # آپدیت متر جدید
                        meter_sparsityloss.update(reduced_sparsity_loss.item(), n)
            ...
            if self.rank == 0:
                ...
                # لاگ کردن زیان جدید در TensorBoard
                self.writer.add_scalar("train/loss/sparsity_loss", meter_sparsityloss.avg, global_step=epoch)
                ...
                self.logger.info(
                    ...
                    # لاگ کردن مقدار زیان جدید
                    "SparsityLoss {sparsity_loss:.4f} "
                    ...
                    "Train_Acc {train_acc:.2f}".format(
                        ...
                        # لاگ کردن مقدار زیان جدید
                        sparsity_loss=meter_sparsityloss.avg,
            ...
            if self.rank == 0:
                ...
                meter_top1_val = meter.AverageMeter("Acc@1", ":6.2f") # نام متر را تغییر دادم تا با متر train تداخل نداشته باشد
                with torch.no_grad():
                    with tqdm(total=len(self.val_loader), ncols=100) as _tqdm:
                        ...
                        for images, targets in self.val_loader:
                            ...
                            prec1 = 100. * correct / images.size(0)
                            n = images.size(0)
                            meter_top1_val.update(prec1, n)

                            _tqdm.set_postfix(
                                val_acc="{:.4f}".format(meter_top1_val.avg),
                            )
                            _tqdm.update(1)
                            time.sleep(0.01)

                Flops = self.student.module.get_flops()
                self.writer.add_scalar("val/acc/top1", meter_top1_val.avg, global_step=epoch)
                ...
                if self.best_prec1 < meter_top1_val.avg:
                    self.best_prec1 = meter_top1_val.avg
                    self.save_student_ckpt(True, epoch)
                else:
                    self.save_student_ckpt(False, epoch) # اصلاح ایندنتاسیون
