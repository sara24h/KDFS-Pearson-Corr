...
class TrainDDP:
    def __init__(self, args):
        ...
        self.target_retention = getattr(args, 'target_retention', 0.5) # نرخ نگه‌داشت هدف (ρ) - پیش‌فرض ۰.۵
        self.lambda_sparse = getattr(args, 'lambda_sparse', 1.0)       # ضریب L_sparsity (λ_sparse) - پیش‌فرض ۱.۰
        # ---------------------------------------------
    ...
    def train(self):
        ...
        if self.rank == 0:
            ...
            # NEW: Add meter for the average L_corr term
            meter_avg_corr = meter.AverageMeter("L_corr", ":.6f") 
            # NEW: Add meter for the average retention rate
        ...
        for epoch in range(self.start_epoch + 1, self.num_epochs + 1):
            ...
            if self.rank == 0:
                ...
                meter_loss.reset()
                meter_top1.reset()
                meter_avg_corr.reset()  # Reset L_corr meter
                meter_retention.reset() # Reset Retention meter
                lr = (
            ...
            with tqdm(total=len(self.train_loader), ncols=100, disable=self.rank != 0) as _tqdm:
                ...
                for images, targets in self.train_loader:
                    ...
                    with autocast():
                        ...
                        # mask_loss here is L_mask = L_corr + lambda_sparse * L_sparsity
                        mask_loss = self.mask_loss(self.student.module)
                        
                        # NEW: Compute L_corr and Retention Rate for logging/monitoring (manual extraction)
                        # We re-run this to isolate the L_corr and Retention metric for logging.
                        total_corr_loss_val = 0.0
                        total_retention_val = 0.0
                        num_layers = 0
                        for m in self.student.module.mask_modules:
                            if isinstance(m, SoftMaskedConv2d):
                                ...
                                # compute_filter_correlation must return L_corr and current_layer_retention
                                corr_loss_val, current_retention_val = compute_filter_correlation(filters, mask_weight, gumbel_temp)
                                
                                total_corr_loss_val += corr_loss_val.item()
                                total_retention_val += current_retention_val.item() # Use .item() since we don't need gradients for logging
                                num_layers += 1
                        
                        avg_corr_loss_val = total_corr_loss_val / num_layers if num_layers > 0 else 0.0
                        avg_retention_val = total_retention_val / num_layers if num_layers > 0 else 0.0
                    ...
                    # NEW: Reduce L_corr and Retention Rate across GPUs
                    reduced_avg_corr = self.reduce_tensor(torch.tensor(avg_corr_loss_val).cuda())
                    reduced_avg_retention = self.reduce_tensor(torch.tensor(avg_retention_val).cuda())
                    ...
                    if self.rank == 0:
                        ...
                        # NEW: Update correlation and retention meters
                        meter_avg_corr.update(reduced_avg_corr.item(), n) 
                        meter_retention.update(reduced_avg_retention.item(), n)
            ...
            if self.rank == 0:
                ...
                self.writer.add_scalar("train/L_corr_loss", meter_avg_corr.avg, global_step=epoch)
                self.writer.add_scalar("train/Retention_Rate", meter_retention.avg, global_step=epoch)
                ...
                self.logger.info(
                    ...
                    "L_corr {avg_corr:.6f} " # NEW: Log L_corr
                    "Retention {retention:.4f}".format(  # NEW: Log Retention
                        epoch,
                        ...
                        retention=meter_retention.avg  # NEW
                    )
                )
                ...
                layer_retentions = [] # Changed from layer_corrs to reflect the expected output from the corrected compute_filter_correlation
                with torch.no_grad():
                    for m in self.student.module.mask_modules:
                        if isinstance(m, SoftMaskedConv2d):
                            ...
                            gumbel_temp = self.student.module.gumbel_temperature
                            # Note: compute_filter_correlation returns L_corr (tensor) and current_layer_retention (scalar)
                            _, current_retention = compute_filter_correlation(filters, mask_weight, gumbel_temp)
                            # Log the retention rate which is a key metric
                            layer_retentions.append(round(current_retention.item(), 4))
                self.logger.info(f"[Layer-wise Filter Retention Rate] Epoch {epoch}: {layer_retentions}")
                self.student.train()
                self.student.module.ticket = False
