...
def create_split_dataset(data_dir: str, output_dir: str, seed: int = 42, train_ratio: float = 0.7, 
                        val_ratio: float = 0.15, test_ratio: float = 0.15):
    """
    Split real_and_fake_face dataset into train/valid/test with reproducibility
    """
    if os.path.exists(output_dir):
        print(f"[INFO] Split dataset already exists at: {output_dir}")
        return output_dir
    
    print(f"[INFO] Creating split dataset from: {data_dir}")
    print(f"[INFO] Ratios - Train: {train_ratio}, Val: {val_ratio}, Test: {test_ratio}")
    ...
    if not os.path.exists(fake_dir) or not os.path.exists(real_dir):
        raise FileNotFoundError(f"Expected folders not found: {fake_dir} or {real_dir}")
    
    # Get all images
    fake_images = sorted([os.path.join(fake_dir, f) for f in os.listdir(fake_dir) 
                         if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
    real_images = sorted([os.path.join(real_dir, f) for f in os.listdir(real_dir) 
                         if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
    
    print(f"[INFO] Found {len(fake_images)} fake images and {len(real_images)} real images")
    
    # Split fake images
    fake_train, fake_temp = train_test_split(fake_images, train_size=train_ratio, random_state=seed)
    fake_val, fake_test = train_test_split(fake_temp, train_size=val_ratio/(val_ratio+test_ratio), random_state=seed)
    
    # Split real images
    real_train, real_temp = train_test_split(real_images, train_size=train_ratio, random_state=seed)
    real_val, real_test = train_test_split(real_temp, train_size=val_ratio/(val_ratio+test_ratio), random_state=seed)
    ...
    def copy_files(file_list, dest_dir):
        for src in tqdm(file_list, desc=f"Copying to {dest_dir}"):
            dst = os.path.join(dest_dir, os.path.basename(src))
            shutil.copy2(src, dst)
    
    print("[INFO] Copying files...")
    copy_files(fake_train, os.path.join(output_dir, 'train', 'fake'))
    copy_files(real_train, os.path.join(output_dir, 'train', 'real'))
    copy_files(fake_val, os.path.join(output_dir, 'valid', 'fake'))
    copy_files(real_val, os.path.join(output_dir, 'valid', 'real'))
    copy_files(fake_test, os.path.join(output_dir, 'test', 'fake'))
    copy_files(real_test, os.path.join(output_dir, 'test', 'real'))
    
    print(f"[INFO] Dataset split completed! Saved to: {output_dir}")
    return output_dir
...
def main():
    ...
    parser.add_argument('--dataset_type', type=str, default='wild', choices=['wild', 'real_and_fake_face'], 
                       help='Dataset type: "wild" for 20k wild dataset or "real_and_fake_face" for split dataset')
    parser.add_argument('--real_and_fake_dir', type=str, default='/kaggle/input/real-and-fake-face/real_and_fake_face',
                       help='Path to real_and_fake_face dataset (only used if dataset_type=real_and_fake_face)')
    parser.add_argument('--split_output_dir', type=str, default='/kaggle/working/real_and_fake_face_split',
    ...
    if args.dataset_type == 'real_and_fake_face':
        if is_main:
            # Only rank 0 creates the split
            data_path = create_split_dataset(
                data_dir=args.real_and_fake_dir,
        ...
        dist.barrier()
        data_path = args.split_output_dir
    else:
        # Use wild dataset (original 20k)
    ...
    train_loader, val_loader, test_loader = create_dataloaders_ddp(
        data_path, args.batch_size, rank, world_size
    )
