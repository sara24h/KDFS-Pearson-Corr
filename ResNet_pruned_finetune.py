...
class ResNet_pruned(nn.Module):
    ...
    if self.masks[2] is not None:
        ...
        num_active = active_channels.sum().item()
        
        # Feature map از conv3
        feature_map = self.conv3(out)
        
        # ایجاد padded feature map با صفر
        ...
        # ✅ استفاده از index_add به جای inplace assignment
        active_indices = active_channels.nonzero(as_tuple=False).squeeze(1)
        
        # جایگذاری feature_map در کانال‌های فعال
        for i, idx in enumerate(active_indices):
            padded_feature_map[:, idx] = feature_map[:, i]
