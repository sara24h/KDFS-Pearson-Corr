...
def parse_args():
    parser = argparse.ArgumentParser(description='Convert Sparse Model to Pruned Model using Learnable Masks.')
...
if not os.path.isdir(save_dir):
    print(f"Error: Path '{save_dir}' is not a valid directory.")
    sys.exit(1)
...
def load_pruned_weights(model_pruned, sparse_state_dict, masks):
    ...
    # Copy initial layers and final FC layer
    ...
    for layer_name, num_blocks in layer_configs:
        for block_idx in range(num_blocks):
            ...
            # Handle downsample layers if they exist
...
try:
    # Create output filenames using the input path
    save_path_full = os.path.join(save_dir, 'resnet50_base_pruned_model_learnable_masks.pt')
    save_path_weights_only = os.path.join(save_dir, 'resnet50_base_pruned_weights_learnable_masks.pt')
